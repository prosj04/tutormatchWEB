...
### 24.3 사업 지적점 추가 10선 (2026-07-03 2차 분석, 기존 BR/MK/§24.1과 중복 없음)

1. **가격 앵커링 부재** — 회당 단가(9~10만원)로만 표시되어 학부모가 월 총액(72~108만원+)을 체크아웃에서 처음 체감. 대형 학원·저가 매칭 대비 "전담 매니저+관리 포함" 패키지 가치 프레임으로 총액을 미리 정당화해야 이탈이 준다.
2. **LTV/코호트 지표 부재** — 재결제 기능(BR-1) 이전에 목표 리텐션(예: 3개월+)조차 정의가 없다. 결제 코호트별 잔존율을 어드민에서 추적 못 하면 CAC 상한을 계산할 수 없어 유료 마케팅을 태울 수 없다.
3. **선생 리텐션 인센티브 없음** — 선생이 떠나면 담당 학생이 연쇄 이탈한다. 인수인계 절차(BR-21)와 별개로, 정산 투명성·장기 활동 등급·재매칭 우선권 같은 유지 장치가 없다.
4. **성과 데이터가 축적되지 않는 '프리미엄'** — 성적/모의고사 before-after를 제품이 수집하지 않는다. 월간 리포트에 성적 입력 필드만 추가해도 "성적 향상 사례"가 마케팅 자산으로 자동 축적된다 (MK-6의 근거 부재를 해결하는 공급원).
5. **추천 루프 부재** — 학부모 구전이 이 시장 최대 채널인데 추천 코드/양측 인센티브(수업 1회 추가 등)가 없다. CAC가 가장 싼 채널을 방치 중.
6. **상담 일정 조율이 수동** — 대면 상담 일정을 매니저가 수동 조율(노쇼 대응 포함). 캘린더 연동·리마인더·노쇼 시 자동 재예약이 없어 매니저 시간이 상담이 아닌 조율에 소모된다 (BR-3 병목을 악화).
7. **입시 계절성 무대응** — 수요는 새학기(3월)·여름방학·수능 D-100에 몰린다. 피크 대비 선생 공급 선확보와 시즌 캠페인 계획이 없으면 피크에 매칭 SLA가 무너지고 비수기에 공급이 논다.
8. **환불 보장 남용 방어 없음** — "첫 수업 100% 환불"은 전환 장치지만 1인 1회 한정·재가입 시 미적용 규정과 환불율 모니터링 지표가 없다. 환불율이 X%를 넘으면 보장 자체가 역마진이 되는데 그 X를 아무도 모른다.
9. **과목 포지셔닝 미정** — 스키마는 과목 단위인데 어느 과목부터 선생 풀을 깊게 팔지 전략이 없다. 얕고 넓은 풀은 매칭 품질(=핵심 가치)을 희석한다. 초기엔 수학 등 1~2과목 집중이 정석.
10. **'매니저 케어'가 보이지 않는 해자** — 저가 매칭(김과외·설탭류)과의 유일한 차별점이 매니저 관리인데, 그 활동(상담 기록, 개입, 리포트)이 학부모에게 가시화되지 않으면 가격 비교로 끌려간다. 케어 로그·개입 알림의 상품화가 곧 방어선 (MK-8이 카피 문제라면 이것은 제품 문제).

### 24.4 사업 지적점 추가 20선 (2026-07-03 3차 분석, 기존 항목과 중복 없음)

**전환·성장 (Go-to-market)**

1. **`vercel.app` 도메인으로 영업 중** — 프로덕션이 `tutormatch-web.vercel.app`. 커스텀 도메인 없이는 학부모 신뢰·SEO·광고 심사(네이버/카카오/메타 광고는 도메인 요구) 전부 불리. 월 100만원짜리 서비스가 무료 호스팅 주소를 쓰는 인상.
2. **브랜드 분산** — 서비스명은 콘코드(Concord), 도메인은 tutormatch, 리포 이름은 premium-tutoring. 검색·구전·광고가 한 이름으로 쌓이지 않음. 도메인 확보 시 브랜드 통일 필요.
3. **네이버 생태계 부재** — 한국 학부모 시장의 발견 채널은 네이버(맘카페·블로그·플레이스)인데 콘텐츠 SEO·지역 키워드("분당 수학 과외") 대응이 없음. 구글 SEO보다 우선순위가 높은 시장.
4. **소셜 로그인 부재** — `auth.ts`는 CredentialsProvider(이메일/비번)만. 40대 학부모에게 회원가입 폼은 그 자체가 이탈 지점. 카카오 로그인 하나로 가입 전환이 구조적으로 달라짐.
5. **미결제 리드 너처링 부재** — 무료 상담에서 수집한 진단 정보가 상담 후 미결제 리드의 팔로업(D+3 문자, 진단 리포트 발송)에 전혀 재사용되지 않음. 상담→결제 사이에 이탈한 리드가 그대로 증발.
6. **상담 전환이 치프 개인기** — 상담 스크립트·진단 리포트 양식이 표준화되어 있지 않아 전환율이 사람에 종속. 매니저 증원 시 전환율이 복제되지 않음.

**가격·수익 구조**

7. **번들 레버 부재** — 요금이 단순 배수(회당 단가 × 회수 × 과목, `pricing-plans.ts`)라 2과목·형제 등록 할인이 없음. 객단가를 올릴 가장 싼 레버(2과목 5% 등)를 방치. 과목 수도 최대 2로 하드코딩.
8. **일시정지(pause) 정책 없음** — 시험 직후·가족여행·방학 초에 "한 달 쉬고 싶다"는 수요는 필연. pause 옵션이 없으면 전부 해지로 빠지고, 재개 마찰이 커짐.
9. **결제 실패 dunning 플로우 없음** — 재결제(BR-1) 도입 시 카드 실패·잔액 부족 대응(재시도 스케줄·유예 기간·서비스 정지 기준)이 설계 안 되면 비자발적 이탈(involuntary churn)이 그대로 매출 손실.
10. **가격 인상 정책 미정** — 회당 단가 구조는 인상이 즉시 체감됨. 기존 고객 가격 보호(grandfathering) 원칙을 미리 정하지 않으면 첫 인상 때 이탈 파도.
11. **현금영수증·증빙 미지원** — 교육비는 증빙 요구가 높은 지출(연말정산 교육비 공제 인식). 현금영수증/세금계산서 자동 발급이 없으면(코드에 부재 확인) 결제 후 CS가 반복 발생.
12. **알림 원가 구조** — 알림이 전부 SMS(Solapi) 단가 기준. 카카오 알림톡 전환 시 건당 원가 ~1/3 + 도달률·클릭률 상승. 알림량이 늘어나는 Phase 2~3 전에 전환해야 원가가 굳지 않음.

**서비스 이행·신뢰**

13. **결제→첫 수업 리드타임 SLA 부재** — 결제 후 첫 수업까지 며칠 걸리는지 정의도 측정도 없음. 프리미엄 결제 후 2주 침묵이면 환불 요청. "결제 후 7일 내 첫 수업" 같은 약속을 상품화하고 초과 시 자동 에스컬레이션.
14. **매칭 실패 플로우 없음** — 조건에 맞는 선생이 없을 때(과목·지역·일정 불일치) 대기/부분 환불/조건 완화 제안 중 무엇을 할지 미정. 매칭 실패율은 공급 부족의 조기 신호인데 측정 안 됨.
15. **선생 교체 정책 미정** — 학생-선생 불화 시 교체 횟수·절차 규정이 없음. 역으로 "선생님 무제한 교체 보장"은 결제 장벽을 낮추는 강력한 전환 카피가 될 수 있는 지점.
16. **수업 단위 품질 신호 없음** — 수업이 실제 어땠는지 데이터가 0 (학생 별점·한줄 피드백·선생 수업일지 없음). '매니저 관리'가 실체를 가지려면 수업 단위 신호가 매니저 대시보드로 흘러야 함. 리포트(FI-1)의 원료이기도.
17. **선생 귀책 노쇼·지각 보상 규정 없음** — 선생이 펑크 낼 때 보강+보상(수업 1회 추가 등) 기준이 없으면 신뢰 훼손이 즉시 환불로 이어짐. 선생 계약서에 페널티 조항도 필요.

**운영·리스크**

18. **학습 목표 합의서 부재** — 첫 상담에서 목표(성적·습관)를 문서화해 월간 리포트와 대조하는 '약속 문서'가 없음. 기대치 관리 실패는 프리미엄 서비스 환불의 최대 원인 — 목표 합의서는 환불 방어이자 리포트의 뼈대.
19. **웹/앱 채널 역할 미정의** — 결제는 웹 전용, 수락·QnA는 앱 중심으로 흘러왔는데 채널별 역할 원칙이 없어 기능마다 이중 구현·drift가 반복됨 (§19 journey drift가 그 증상). "학부모=웹, 학생=앱" 같은 원칙을 정하면 개발 비용이 절반.
20. **백업·사업 연속성 미확인** — Supabase 단일 의존. PITR(시점 복구) 활성화 여부·백업 주기·복구 절차가 미확인. 결제·학습 기록이 법적 보관 대상(5년)인 만큼 DB 유실은 사업 종료급 리스크.
