> **⚠️ 마스터 핸드오프는 `/HANDOFF.md` 입니다.** 신규 세션은 그쪽을 먼저 읽으세요.
> 이 문서는 세션 로그(§25)와 이연 스펙(§26)의 **원본**으로 보존됩니다 — 편집 시 §25 스타일로 append.
> HANDOFF.md와 상충하면 최신 세션 로그가 진실입니다. 자세한 문서 지도: `docs/README.md`.
> 갱신 정책: 이 파일의 §0~§24는 아카이브 성격(구버전 표기 포함). §25~§26만 세션마다 계속 확장.
> (이 5줄은 2026-07-04 핸드오프 재정비 시 추가된 안내 블록 — 아래 원본은 손대지 않습니다.)
...
### 26.0 오너 결정 기록 (2026-07-04)

- **Q2 = A 확정**: Toss 빌링키 자동결제 도입. 단 "언제든 해지 가능"을 전면 어필 — 셀프서비스 해지(다음 결제 중단)가 마케팅 클레임의 전제이므로 필수 구현.
- **Q3 확정**: PG 지급대행 분배 구조. 선생 시급 30,000원, 회사는 학부모로부터 시간당 50,000원 기준(정가 앵커)으로 수취, 플랜 가격에 할인 반영.
- **신규 요금제 (기존 회당제 교체)**: 중등|고등 × 4플랜 월정액 — 주1회 2h: 380,000|430,000 / 주1회 3h: 550,000|580,000 / 주2회 2h: 760,000|780,000 / 주2회 3h: 1,060,000|1,120,000. 금액 8종 전부 고유 → `planIdFromAmount` 역매핑 유지 가능. ⚠️ 구현 가정: 과목 수 배수 폐지(시간 기반, 과목은 상담에서 조율) — 오너 미확인 가정이므로 이견 시 정정.
- **#29 학부모 앱**: 대기 유지.
