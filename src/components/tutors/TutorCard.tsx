...
function Stars({ value }: { value: number }) {
  ...
  return (
    <span className="flex items-center gap-0.5 text-gold" aria-label={`평점 ${value}`}>
      {Array.from({ length: full }).map((_, i) => (
        <span key={i}>★</span>
      ))}
      {half && <span className="text-gold/40">★</span>}
      ...
      <div className="flex flex-1 flex-col p-6 md:p-8">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {t.subjects.slice(0, 3).map((s) => (
            <span
              key={s}
              className="rounded-full bg-gold/15 px-2.5 py-1 text-xs font-medium text-navy"
            >
              {s}
            </span>
          ))}
        ...
        {degree ? (
          <p className="mt-3 text-xs text-text-mid">{degree}</p>
        ) : null}
        ...
        <Link
          href={`/tutors/${t.id}`}
          className="mt-6 inline-flex w-full items-center justify-center rounded-2xl border-2 border-navy py-3.5 text-sm font-semibold text-navy transition hover:bg-navy hover:text-white"
        >
          프로필 보기
        </Link>
      </div>
