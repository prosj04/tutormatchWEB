...
## 11. 통합 4역할 디자인 적용 라운드 검증 (2026-07-11, opus 5팀 중 3팀 완료 + 본체 수정)

> 대상: 이번 세션 대규모 변경분 — 웹 포털(학부모/선생님/매니저/어드민/학생 4p)·헤더 세션·구독 일시정지 정책, 모바일 4역할 앱. 5팀 투입 중 2팀(웹 포털 권한·디자인 계약)은 사용자 중단으로 미완 — 해당 영역은 tsc/eslint/next build로 기본 커버, 필요 시 후속 라운드.

### 11.1 신규 수정 항목

- **[P1] 학부모에게 일시정지 노출**: `parent-data.ts`가 ACTIVE 구독만 조회 → PAUSED 자녀는 플랜 소멸·"매칭 중" 후퇴 표기(정책 위반). `status: { in: ["ACTIVE","PAUSED"] }`로 확대 + 모바일 `(parent)/payments.tsx`의 active 판정에 PAUSED 포함. 라벨 마스킹(§구독중 표기)은 기존 수정으로 유효.
- **[P2] 모니터링 구독 선택 ACTIVE 우선**: 학생당 1건 채택이 createdAt만 기준 → ACTIVE·PAUSED 동시 존재 시 오선택 가능. ACTIVE 우선 채택으로 변경(`manager-portal-data.ts`).
- **[P2] 완료 상담의 취소 버튼**: 모바일 매니저 내담당 카드에서 COMPLETED에도 취소 노출(서버 404→일반 오류). COMPLETED면 버튼 숨김(`(manager)/index.tsx`).
- **[P2] 미승인 강사 잠긴 탭 안내**: 학생/질문 탭 403이 "불러오기 실패"로 표시. "승인 후 이용할 수 있어요" 잠금 EmptyState 분기(`(teacher)/students.tsx`·`questions.tsx`, 기존 `pending` 필터 변수와 충돌 → `approvalPending`).
- **[P2] 코스메틱 2건**: 모바일 학부모 프로필 GET에 email 미반환(→ user 조회 포함), link 성공 응답에 child 이름 없음(→ `child:{name,grade}` 포함) — 클라 기대 필드와 계약 일치화.

### 11.2 확인 후 미수정 (설계상 정상/비이슈)

- **role 미저장 구버전 사용자 게이트 폴백**: PARENT/TEACHER/MANAGER는 전부 이번 세션 신규 로그인 경로로만 진입(로그인 시 saveRole) → 실발생 없음. 역할 변경 시 refresh 토큰은 새 role로 재발급되나 로컬 role 미갱신 → 다음 401 루프 후 로그아웃 수렴(재로그인 유도, 허용).
- **모바일 결제 CTA의 웹 세션 분리**: 기존 학생 공통 한계(웹 /checkout 위임 설계), 신규 결함 아님.
- **일시정지 경계·크론**: 클라 1~35일 검증과 서버 상한 정합, AUTO-RENEWAL PAUSED 이중 제외, RESUME 연장 로직 정합 — 이상 없음 확인.
- **모바일 API 계약**: 선생님·매니저 전 엔드포인트 요청/응답 전수 대조 일치. 질문 N+1 집계는 정확(전용 엔드포인트는 개선 후보로만 기록).
