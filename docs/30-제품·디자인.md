...
> **현행 적용 기준 (2026-09-17)**: 사업은 전담 매니저 중심의 프리미엄 1:1 방문 과외 큐레이션·학습관리 서비스다. [사업 방향](BUSINESS_DIRECTION.md)이 소개·배정·요금 원칙의 정본이다. 아래 과거 진단·제안·구현 상태는 작성 시점의 기록이며 현행 정책이나 현재 검증 결과로 간주하지 않는다. 제안의 채택·조건은 개선·확장 마스터를 따른다. 원문의 수치와 결정 이력은 보존한다.
...
### 상충 군집 — 오너 정본 확정 (2026-07-14)

같은 문서 안에서 **처방이 정면 상충**했던 5건. 오너 결정으로 정본 확정. 공개 페이지 코드는 동결이므로 아래는 **문서 정본**이며, 향후 판단·핸드오프는 이 표를 따른다.

| # | 상충 축 | 정본 (확정) | 폐기·미결 |
|---|---|---|---|
| 15 | 모바일 하단 CTA·환불 배지 | **E:#4 환불 배지 노출 유지** (신뢰장치, 할인 아님) | C:사이클1#3 배지 제거 → 폐기 |
| 19 | 선생님 이미지 방향 | **결정대기** — 양안(1:2-1 실사진 확대 / B:§2·E:#6 축소·아바타) 문서 병존 | — |
| 20 | 히어로 우측 처리 | **B:§2 단일 사진 확정** | E:#1 프루프 스택 → 폐기 |
| 21 | 요금 CTA 수렴 정책 | **B:§6 상담 수렴 유지** (큐레이션 세계관) | C:사이클1#2·F:D1 문구 정직화 → 폐기 |
| 22 | 긴급성·혼잡 연출 | **C:§4 원칙2 금지** — 근거 없는 긴급성 금지, 검증 가능한 사실만 (카피 헌법 정합) | B:§3 "긴급성 연출 허용(07-06)" → 폐기 (본문 반영됨) |

> 군집 23(헤더 색 테마 스위처)은 양쪽 다 해소 기록 있어 조치 불요. #19만 미결 — 나머지 4건 정본 확정.
...
- ~~**긴급성·수요 연출 허용** (2026-07-06)~~ → **폐기 (2026-07-14 오너, 정본=C §4 원칙2)**: "상담 많아 순차 안내"·"인기 시간대 조기 마감" 등 **근거 없는 긴급성·혼잡 어필 금지**. 검증 가능한 사실만 노출한다 (카피 헌법 정합).
- 존재하지 않는 조사를 구체 수치로 조작("n명 중 m명, ○년 조사")하는 것도 당연히 금지(표시광고법).
...
8. **Process 섹션 2단화** — 좌측 5단계, 우측 **진행 알림 목업 스택** (상담 접수 · 방문 상담 확정 · 선생님 배정 · 첫 수업 안내). ~~"수락하고 첫 수업 잡기" 버튼 목업·학생 수락 플로우 시각화~~ — 폐기 (2026-07-14 오너, 북극성 6·큐레이션 세계관: 수락은 형식 절차, 마케팅·목업 강조 금지. 배정=매니저 케어 서사로 표현).
