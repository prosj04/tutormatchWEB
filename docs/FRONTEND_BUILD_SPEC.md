...
> **웹 `/parent` 스켈레톤 이미 존재(기능+뼈대):** `src/app/parent/`에 최소 기능 페이지가 배포됨 — 레이아웃(가드 `requireParentPage`), 자녀 목록+연결코드 입력(`page.tsx`), 자녀별 리포트(`children/[studentId]/reports`), 결제(`payments`), 상담 신청(`consultation`). 모두 위 API에 연결. 디자인 폴리시는 미적용 — 프론트/디자인에서 교체·보강.
